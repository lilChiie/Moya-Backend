...
def create_recommendation():
    ...
    if not isinstance(
        ...
        return jsonify({
            "success": False,
            "message": (
                "accessibility_ids must be an array"
            )
        }), 400


    if len(accessibility_ids) == 0:
    ...
    found_accessibility_ids = {
        accessibility.id
        for accessibility
        in accessibilities
    }

    invalid_accessibility_ids = [
        accessibility_id
        for accessibility_id
        in accessibility_ids
        if accessibility_id
        not in found_accessibility_ids
    ]

    if invalid_accessibility_ids:
    ...
    try:
        ...
        for result_data in saved_results:

            dss_result = (
                result_data["dss_result"]
            )
